...
export function CartProvider({children}:{children:ReactNode}){
  ...
  const addItem=useCallback((item:Omit<CartItem,"quantity">,quantity=1)=>{const current=safeRead();const found=current.find(x=>x.id===item.id);const max=Math.max(1,item.maxQuantity??999);const next=found?current.map(x=>x.id===item.id?{...x,...item,quantity:Math.min(max,x.quantity+quantity)}:x):[...current,{...item,quantity:Math.min(max,Math.max(1,quantity))}];persist(next);if(found)notify.cart.updated(item.name);else notify.cart.added(item.name)},[persist]);
  const removeItem=useCallback((id:string)=>{const current=safeRead();const removed=current.find(x=>x.id===id);persist(current.filter(x=>x.id!==id));if(removed)notify.cart.removed(removed.name)},[persist]);
  const updateQuantity=useCallback((id:string,quantity:number)=>{const current=safeRead();const item=current.find(x=>x.id===id);persist(current.map(x=>x.id===id?{...x,quantity:Math.max(1,Math.min(x.maxQuantity??999,quantity))}:x));if(item)notify.cart.updated(item.name)},[persist]);
