...
function buildData(phone = "01711111111"): ProfileDashboardData {
  const records = {
    profile: [],
    balance: ["Balance added", "Order payment", "Refund", "Cashback", "Promotional credit", "Withdrawal"].map((title, index) => ({ id: `TXN-${index + 1}`, title, subtitle: index % 2 ? "bKash wallet transaction" : "Arogga balance update", status: index % 3 ? "Completed" : "Pending", amount: [1200, -450, 220, 90, 150, -300][index], date: `2026-07-${20 - index}` })),
    offers: ["Health saver coupon", "Bank card discount", "Payment cashback", "Baby care deal", "Expiring wellness offer"].map((title, index) => ({ id: `OFFER-${index + 1}`, title, subtitle: "Copy, save, apply, or view eligible products", status: index === 4 ? "Expiring Soon" : "Eligible", amount: 100 + index * 50, date: `2026-08-${10 + index}` })),
    orders: [],
    ...
    addresses: ["Home address", "Office address", "Hospital delivery"].map((title, index) => ({ id: `ADDR-${index + 1}`, title, subtitle: "Mirpur, Dhaka, Bangladesh", status: index === 0 ? "Default" : "Saved", meta: "Detect location and pin map ready" })),
    patients: ["Mohammed Tamim Hasan", "Mother", "Father"].map((title, index) => ({ id: `PAT-${index + 1}`, title, subtitle: index ? "Family patient profile" : "Default patient", status: index ? "Private" : "Default", meta: "Blood group, allergies, conditions, prescriptions" })),
    ...
    reports: ["Order delivery problem", "Payment refund issue", "Prescription support"].map((title, index) => ({ id: `RPT-${index + 1}`, title, subtitle: "Ticket conversation with support", status: ["Open", "In Progress", "Resolved"][index], date: `2026-07-${10 + index}` })),
